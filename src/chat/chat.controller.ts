...
export class ChatController {
  ...
  @Get('conversations')
  @ApiOperation({ summary: 'Listar minhas conversas com último msg e unread count' })
  ...
  @ApiOperation({ summary: 'Buscar mensagens (polling). Use ?since=ISO para incremental' })
  @ApiParam({ name: 'bookingId', description: 'UUID da reserva' })
  @ApiQuery({ name: 'since', required: false, description: 'ISO timestamp para polling incremental' })
  ...
  getMessages(
    ...
    return this.chatService.getMessages(bookingId, req.user.sub, since, limit || 50);
  }
