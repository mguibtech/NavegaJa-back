...
export class FavoritesController {
  ...
  @Post()
  @ApiOperation({
    summary: 'Adicionar item aos favoritos (destino, barco ou capitão)',
  })
  ...
  @ApiQuery({
    ...
    enum: FavoriteType,
    required: false,
    description: 'Filtrar por tipo (destination, boat, captain)',
  })
  @ApiOkResponse({ type: [FavoriteResponseDto] })
  findAll(
  ...
  check(@Request() req: AuthenticatedRequest, @Body() dto: CreateFavoriteDto) {
    return this.favoritesService.check(req.user.sub, dto);
  }

  @Post('toggle')
  @ApiOperation({ summary: 'Adicionar ou remover favorito (toggle)' })
  toggle(@Request() req: AuthenticatedRequest, @Body() dto: CreateFavoriteDto) {
