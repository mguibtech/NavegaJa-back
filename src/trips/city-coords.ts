/**
 * Lookup table de coordenadas das principais cidades e municipios da regiao.
 * Usada para geocodificar origin/destination e sugerir localidades.
 */
import {
  normalizeLocationText,
  preferCanonicalLocationName,
  titleCaseLocationText,
} from '../locations/location-text';

type CityCoords = { lat: number; lng: number };

type CanonicalCityEntry = CityCoords & {
  normalizedName: string;
  displayName: string;
};

const RAW_AMAZON_CITY_COORDS: Record<string, CityCoords> = {
...
function buildCanonicalCityEntries(
  source: Record<string, CityCoords>,
): CanonicalCityEntry[] {
  const canonicalByNormalizedName = new Map<string, CanonicalCityEntry>();

  for (const [rawName, coords] of Object.entries(source)) {
    const normalizedName = normalizeLocationText(rawName);
    const displayCandidate = titleCaseLocationText(rawName);
    const existing = canonicalByNormalizedName.get(normalizedName);

    canonicalByNormalizedName.set(normalizedName, {
      normalizedName,
      displayName: preferCanonicalLocationName(
        existing?.displayName ?? null,
        displayCandidate,
      ),
      lat: existing?.lat ?? coords.lat,
      lng: existing?.lng ?? coords.lng,
    });
  }

  return Array.from(canonicalByNormalizedName.values());
}

export const AMAZON_CITY_SUGGESTIONS: ReadonlyArray<CanonicalCityEntry> =
  Object.freeze(buildCanonicalCityEntries(RAW_AMAZON_CITY_COORDS));

export const AMAZON_CITY_COORDS: Readonly<Record<string, CityCoords>> =
  Object.freeze(
    Object.fromEntries(
      AMAZON_CITY_SUGGESTIONS.map(({ normalizedName, lat, lng }) => [
        normalizedName,
        { lat, lng },
      ]),
    ),
...
/**
 * Normaliza o nome da cidade para lookup.
 * Retorna coordenadas ou null se nao encontrado.
 */
export function geocodeCity(
  ...
  const normalizedName = normalizeLocationText(cityName);
  if (AMAZON_CITY_COORDS[normalizedName]) {
    return AMAZON_CITY_COORDS[normalizedName];
  }

  for (const [knownName, coords] of Object.entries(AMAZON_CITY_COORDS)) {
    if (
      normalizedName.startsWith(knownName) ||
      knownName.startsWith(normalizedName)
    ) {
      return coords;
    }
  }
