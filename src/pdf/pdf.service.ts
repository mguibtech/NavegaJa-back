...
import { PdfDocumentCtor, PdfStream } from './pdf.types';
...
export class PdfService {
  ...
  createTicket(data: {
    ...
    doc
      .fontSize(10)
      .fillColor('#333333')
      .text(`Reserva #${shortId}`, 350, 50, { align: 'right', width: 195 });
    doc
      .fontSize(8)
      .fillColor('#888888')
      .text(`Emitido: ${formatDate(data.createdAt)}`, 350, 64, {
        align: 'right',
        width: 195,
      });
    ...
    if (data.estimatedArrivalAt) {
      y += 15;
      doc.text(
        `Chegada prevista: ${formatDate(data.estimatedArrivalAt)}`,
        50,
        y,
      );
    }
    ...
    doc
      ...
      .font('Helvetica')
      .fillColor('#333333')
      .text(data.passengerName, 50, y);
    doc.text(
      `${data.captainName} ⭐ ${Number(data.captainRating).toFixed(1)}`,
      col2,
      y,
    );
    ...
    doc
      ...
      .font('Helvetica')
      .fillColor('#333333')
      .text(`${data.boatName} (${data.boatType})`, 50, y);
    ...
    if (hasExtras) {
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#888888')
        .text('PASSAGEIROS ADICIONAIS', 50, y);
      y += 14;
    ...
    if (hasChildren) {
      ...
      data.children!.forEach((c) => {
        const label =
          c.age === 0 ? 'Bebê (0 anos)' : `${c.age} ano${c.age > 1 ? 's' : ''}`;
    ...
    doc
      ...
      .font('Helvetica-Bold')
      .fillColor('#0066CC')
      .text(formatMoney(data.totalPrice), 50, y);
    ...
    doc
      .fontSize(11)
      .font('Helvetica')
      .fillColor(statusColors[data.paymentStatus] || '#333333')
    ...
    if (data.qrCodeCheckin) {
      ...
      doc
        ...
        .fillColor('#555555')
        .text(
          'Apresente este código ao capitão no momento do embarque.',
          50,
          y,
        );
      y += 15;
      ...
      if (data.qrCodeCheckin.startsWith('data:image')) {
        try {
          const base64Data = data.qrCodeCheckin.replace(
            /^data:image\/\w+;base64,/,
            '',
          );
          ...
          doc
            .fontSize(8)
            .fillColor('#333333')
            .text(`ID: ${shortId}`, 160, y + 40);
        } catch {
          doc
            .fontSize(9)
            .fillColor('#333333')
            .text(`Código: ${shortId}`, 50, y);
        }
      } else {
  ...
  createCargoManifest(data: {
    ...
    doc
      ...
      .font('Helvetica')
      .fillColor('#666666')
      .text('Manifesto de Carga', 50, 76);
    doc.moveTo(50, 95).lineTo(545, 95).stroke('#0066CC');

    doc
      .fontSize(10)
      .fillColor('#333333')
      .text(`Viagem #${shortId}`, 350, 50, { align: 'right', width: 195 });
    doc
      .fontSize(8)
      .fillColor('#888888')
      .text(`Emitido: ${formatDate(new Date())}`, 350, 64, {
        align: 'right',
        width: 195,
      });
    ...
    doc
      ...
      .fillColor('#555555')
      .text(
        `Partida: ${formatDate(data.departureAt)}   |   Capitão: ${data.captainName}   |   Embarcação: ${data.boatName}`,
        50,
        y,
      );
    ...
    for (const s of data.shipments) {
      ...
      doc
        ...
        .font('Helvetica-Bold')
        .fillColor('#333333')
        .text(s.trackingCode, 50, y);
      doc
        .fontSize(7)
        .font('Helvetica')
        .fillColor('#555555')
        .text(`${s.senderName} → ${s.recipientName}`, 130, y, {
          width: 230,
          ellipsis: true,
        });
    ...
    doc
      ...
      .fillColor('#AAAAAA')
      .text(
        'NavegaJá — Documento interno. Manifesto de carga emitido para controle da viagem.',
        50,
        y,
        { align: 'center', width: 495 },
      );
